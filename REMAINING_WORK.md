...
| 5.5 verifier attestation: `WS_RP_SM_RpIntegrity__008`–`__012`, `WS_RP_MS_Metadata__116`–`__124` | done | `client_id_scheme: "verifier_attestation"` signs under the Section 5.10 prefix with the attestation in the `jwt` JOSE header, and the `verifier_attestation` object breaks one binding at a time. `RpIntegrity__010` still needs an operator to configure the fixture issuer key, published at `/openid4vp/verifier-attestation-issuer/jwks.json`, as trusted in the wallet |
| 5.5 verifier info attestations: `WS_RP_SM_RpIntegrity__001` | done | Caller-supplied, see 6.3 |
