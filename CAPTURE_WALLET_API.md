...
| `redirect_uri` | Absolute URI for the Wallet to open after a successful presentation; use `{{base_url}}/{{nonce}}/redirect` for a capture redirect page | — |
...
| `GET` | `/{nonce}/redirect?response_code=...` | Service-hosted capture redirect page created from the `redirect_uri` template. |
