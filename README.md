...
* `client_metadata` may be an object to replace the generated verifier metadata, or `null` to omit the parameter. Omission is supported only with `direct_post`. For `direct_post.jwt` a replacement must publish the session's generated encryption public key, compared by RFC 7638 thumbprint so `alg`, `use`, and `kid` may be altered or omitted. That key is minted inside the same call that returns it, so pair a `direct_post.jwt` replacement with `allow_undecryptable_response`.
* `allow_undecryptable_response` is a test-only flag. With `true` the service publishes the supplied `client_metadata` for `direct_post.jwt` even though its `jwks` is not the verifier's encryption key, which is what wallet response-encryption negative tests need: a JWK without `alg`, with an `alg` other than `ECDH-ES`, or a static key reused across sessions. The service can then no longer decrypt a response, a wallet that answers anyway is captured as a decryption failure, and the session records a `vp_undecryptable_response_allowed` event. It requires a `client_metadata` object.
