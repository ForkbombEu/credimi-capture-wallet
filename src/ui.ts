...
function appCss(): string {
  return [
    ...
    ".metadata-row summary::after { content: '+'; display: inline-grid; place-items: center; width: 20px; height: 20px; flex: 0 0 auto; color: var(--brand-primary); font-family: JetBrains Mono, Fira Code, Cascadia Code, ui-monospace, monospace; font-size: 18px; font-weight: 500; line-height: 1; }",
    ".metadata-row[open] summary { border-bottom: 1px solid var(--border); }",
    ".metadata-row[open] summary::after { content: '\u2212'; }",
    ...
    "@media (max-width: 860px) { h1 { font-size: 32px; } h2 { font-size: 22px; } .container, .topbar-inner, .hero-inner, .footer-inner { padding-left: 16px; padding-right: 16px; } .hero-band { padding: 40px 0; } .hero-inner, .session-layout { grid-template-columns: 1fr; } .session-header, .section-head, .footer-content { flex-direction: column; align-items: stretch; } .session-actions { align-items: stretch; flex-direction: column; } .session-actions .btn { width: 100%; } .footer-links { justify-content: flex-start; } .qr-box { width: 100%; max-width: 336px; } }",
  ].join("\n");
}
